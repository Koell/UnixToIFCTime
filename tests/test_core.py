...
def test_ifc_to_unix_year_day():
    from ifcdate.core import ifc_to_unix
    from datetime import datetime, timezone

    assert ifc_to_unix("2025-13-29") == int(datetime(2025, 12, 31, tzinfo=timezone.utc).timestamp())


def test_ifc_to_unix_invalid_day_gt_30_in_month_13():
    with pytest.raises(ValueError, match="Invalid day 31 in month 13"):
        ifc_to_unix("2025-13-31")
    with pytest.raises(ValueError, match="Invalid day 32 in month 13"):
        ifc_to_unix("2025-13-32")

def test_unix_to_ifc_fallback_unsupported_format():
    result = unix_to_ifc(0, out_format="%unsupported")
    assert result == "1970-01-01"

def test_unix_to_ifc_year_day_and_regular_day():
    from ifcdate.core import unix_to_ifc
    from datetime import datetime, timezone

    # 31.12.2025 ist der 365. Tag (Nicht-Schaltjahr) -> Year Day
    dt_year_day = datetime(2025, 12, 31, tzinfo=timezone.utc)
    unix_year_day = int(dt_year_day.timestamp())
    assert unix_to_ifc(unix_year_day) == "2025-13-29"

    # 1.3.2025 ist ein regulärer Tag (Nicht-Schaltjahr) -> else-Zweig
    dt_regular = datetime(2025, 3, 1, tzinfo=timezone.utc)
    unix_regular = int(dt_regular.timestamp())
    assert unix_to_ifc(unix_regular) == "2025-03-04"
